...
public class LITPpackageapiTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPpackageapiTestRunner.class);
    ...
    @TestId(id = "CXP9031120-1", title = "Upgrade ERIClitppackageapi rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitppackageapiRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("packageapi");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031120-2", title = "Run python test cases for ERIClitppackageapi package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitppackageapiTests() {
     ...
     * @PRE Execution of test {@link #runERIClitppackageapiTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031120-3", title = "Parse xml outputs from python tests")
